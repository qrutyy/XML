...
let compile_to_asm ~enable_gc src out_ppf =
  ...
  | Ok ast ->
    (match anf_program ast with
     | Error e ->
       ...
       exit 1
     | Ok anf -> gen_program ~enable_gc out_ppf anf)
;;
...
let () =
  ...
  in
  match !out_file with
  | None -> compile_to_asm ~enable_gc:!enable_gc src Format.std_formatter
  | Some path ->
    let oc = open_out path in
    let ppf = Format.formatter_of_out_channel oc in
    compile_to_asm ~enable_gc:!enable_gc src ppf;
