...
static uint64_t *STACK_SCAN_LOW = NULL;  /* range we are scanning in collect() */
...
void collect(void) {
    ...
    if (!PTR_STACK || current_stack_top > PTR_STACK) return;
...
void allocate_banks(void) {
    for (int i = 0; i < 2; i++) {
        GC.start[i] = malloc(size_heap * sizeof(uint64_t));
...
uint64_t *gc_alloc(size_t words, uint64_t tag) {
    ...
    if (GC.alloc_ptr + total_words > GC.end[GC.current_bank] &&
        (collect(), GC.alloc_ptr + total_words > GC.end[GC.current_bank])) {
        fprintf(stderr, "Out of memory\n");
        abort();
    }
    box_header_t *header = (box_header_t *)GC.alloc_ptr;
    *header = (box_header_t){ .tag = (uint8_t)tag, .size = (uint16_t)words };
...
eml_value collect(void) { return (eml_value)(uintptr_t)tag_int_val(0); }
void init_gc(void) {}
void destroy_gc(void) {}
int64_t get_heap_start(void) { return tag_int_val(0); }
int64_t get_heap_final(void) { return tag_int_val(0); }
void set_ptr_stack(uint64_t *stack_bottom) { (void)stack_bottom; }
static void *eml_alloc(size_t bytes, uint64_t tag) { (void)tag; return malloc(bytes); }
#endif
...
#else
  closure *dst = (closure *)malloc(sz);
#endif
  memcpy(dst, src, sz);
  return dst;
}

void *apply1(closure *c, int64_t arg) {
  void *argv[1];
  argv[0] = (void *)(uintptr_t)arg;
  return eml_applyN(c, 1, argv);
}
...
#else
    void **all_args = (void **)malloc((size_t)c->arity * sizeof(void *));
#endif
...
typedef struct { int64_t arity; void *args[]; } tuple;
...
#else
  tuple *t = (tuple *)malloc(words * sizeof(uint64_t));
#endif
  t->arity = argc;
